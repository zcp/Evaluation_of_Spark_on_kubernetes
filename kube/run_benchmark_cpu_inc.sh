...
run_kmeans(){
   ...
   driver_memory=2g
   unit=m
   executor_request_cores=$request_core$unit
   executor_memory=2g
   instance=$instance_num
...
test_instance(){
   ...
   for count in 4 5; do
      for executor_num in 5 10; do
	   for run_benchmark in run_kmeans run_wordcount run_sort; do
              for request_core in 1000 1100 1200 1500 2000; do
	       ...
	       $run_benchmark
	       clean_monitor
	       storage=I$executor_num-$count-$request_core-$run_benchmark$suffix
	       echo $storage
	       transFiles
               delDriverPods
	       #delFiles
