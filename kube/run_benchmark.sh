...
run_net_test(){
   app_executor_dir=/tmp/spark_benchmark_kubernetes/net_test

   code=$hdfs_dir/run_net_test/net_test.py
   data=$hdfs_dir/run_net_test/lda_wiki1w_1
   ...
   { sleep 60s ; kubectl get pods -o wide | grep net_test > $monitor_dir/net_test_pods_distribution; python $benchmark_dir/pod_startup.py $save_startup ; } &
   wait
}
...
test_net_test(){
   sync_time
   for count in 1 ; do
      for executor_num in 1; do
           for run_benchmark in run_net_test; do
               instance_num=$executor_num
               client_num=1
              #for data_size in 4440; do
               ...
               $run_benchmark
               clean_monitor
               storage=I$executor_num-$count-1000-$run_benchmark
               echo $storage
               transFiles
               delDriverPods
               #delFiles
...
test_instance(){
   ...
   for count in 1 2 3 4 5; do
      for executor_num in 5 10 15 20 24; do
	   for run_benchmark in run_kmeans run_kmeans_nocache; do
           #for run_benchmark in run_wordcount; do
               instance_num=$executor_num
               client_num=1
               for data_size in 10240; do
               data=$hdfs_dir/$run_benchmark/$data_size
	       delFiles
	       clear_cache
	       delDriverPods
	       ...
	       $run_benchmark
	       clean_monitor
	       storage=I$executor_num-$count-$data_size-$run_benchmark
	       echo $storage
	       transFiles
               delDriverPods
	       #delFiles
...
#run_pagerank
#delDriverPods
#python $benchmark_dir/pod_star:wqtup.py save_startup_time;
