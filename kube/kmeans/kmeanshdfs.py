...
if __name__ == "__main__":
    ...
    logs.append("lines.map,execution time(ms),%s,partition number,%s\n" %(str(end_millis - start_millis),str(partition_num)))
    ...
    logs.append("data.takseSample,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis - tmp_start_millis),str(partition_num)))
    ...
    while tempDist > convergeDist and iter_num < 3:
        ...
        tmp_end_millis = int(round(time.time() * 1000))
        logs.append("data.map,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis - tmp_start_millis),str(partition_num)))

        tmp_start_millis = int(round(time.time() * 1000))
        pointStats = closest.reduceByKey(lambda p1_c1, p2_c2: (p1_c1[0] + p2_c2[0], p1_c1[1] + p2_c2[1]))
        partition_num = pointStats.getNumPartitions()
        tmp_end_millis = int(round(time.time() * 1000))
        logs.append("closest.reduceByKey,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis-tmp_start_millis),str(partition_num)))
        ...
        tmp_end_millis = int(round(time.time() * 1000))
        logs.append("newPointsRDD,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis - tmp_start_millis),str(partition_num)))

        tmp_start_millis = int(round(time.time() * 1000))
        newPoints = newPointsRDD.collect()
        tmp_end_millis = int(round(time.time() * 1000))
        logs.append("newPoints,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis - tmp_start_millis),str(partition_num)))
        ...
        for (iK, p) in newPoints:
            kPoints[iK] = p
        tmp_end_millis = int(round(time.time() * 1000))
        logs.append("KPoints,execution time(ms),%s,partition number,%s\n" %(str(tmp_end_millis - tmp_start_millis),str(partition_num)))
    ...
    end_millis = int(round(time.time() * 1000))
    end_time = str(datetime.now())
    logs.append("total,execution time(ms),%s,start time,%s,end time,%s\n" %(str(end_millis - start_millis),start_time,end_time))


    from pyspark.sql import Row 
    R = Row('ID', 'log')
    # use enumerate to add the ID column
    logs_rdd = spark.createDataFrame([R(i, x) for i, x in enumerate(logs)])
    #logs_rdd = sc.parallelize(logs, 1)
    #logs_rdd.saveAsTextFile(sys.argv[4])
    #logs_rdd.map(_.toString).toDF.write.mode("overwrite").text(sys.argv[4])
    logs_rdd.coalesce(1).write.format("com.databricks.spark.csv") \
  	.mode("overwrite") \
  	.option("header", "true") \
  	.option("delimiter", ",") \
