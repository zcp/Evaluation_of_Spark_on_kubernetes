   ...
   for count in 1 2 3 4 5; do
      for executor_num in 5 10 15 20 24 ; do
         for run_benchmark in run_kmeans; do
                ...
                $run_benchmark
                clean_monitor
                storage=E$executor_num-$count-$data_size-$run_benchmark-nocache
                echo $storage
                transFiles
                delFiles
                done
   ...
   for count in 1; do
      for executor_num in 1 ; do
         for run_benchmark in run_net_test; do
                ...
                copyFiles
                $run_benchmark
                storage=E$executor_num-$count-1000-$run_benchmark-nocache
                echo $storage
                transFiles
                delFiles
          done
