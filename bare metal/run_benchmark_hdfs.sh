...
run_net_test(){
   ...
   result_loc=$monitor_dir/net_test_result
   data=$hdfs_dir/run_net_test/lda_wiki1w_1
   #We submit the program on master node so that the driver will run on the node.
   echo "run net test benchmark"
   master_node=${nodes[0]}
   sudo sshpass -p 123456  ssh $master_node "cd $app_dir/net_test; ./run.sh $driver_cores $driver_memory $executor_cores $executor_memory $total_cores $code $data $result_loc $master_node $client_num"
}
   ...
   for count in 1 2 3 4 5; do
      for executor_num in 5 10 15 20 24 ; do
         for run_benchmark in run_kmeans run_kmeans_nocache; do
                ...
                $run_benchmark
                clean_monitor
                storage=E$executor_num-$count-$data_size-$run_benchmark
                echo $storage
                transFiles
                delFiles
                done
   ...
   for count in 1; do
      for executor_num in 1 ; do
         for run_benchmark in run_net_test; do
                ...
                copyFiles
                $run_benchmark
                storage=E$executor_num-$count-1000-$run_benchmark
                echo $storage
                transFiles
                delFiles
          done
