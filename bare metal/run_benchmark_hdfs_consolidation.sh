...
#node0 is the master
nodes=($slave2_hostname $slave3_hostname $slave4_hostname $slave6_hostname $slave1_hostname)
node_num=${#nodes[@]}
...
test(){
   ...
   for count in 1 2 3 4 5; do
      for client_num in 1 6; do
         ...
         for run_benchmark in run_wordcount run_sort run_kmeans run_sql_join; do
                executor_num=3
                ...
                $run_benchmark
                clean_monitor
                storage=E$client_num-$count-1000-$run_benchmark$suffix
                echo $storage
                transFiles
                delFiles
          done
